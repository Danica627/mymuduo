...
//channel的tie方法什么时候调用过？一个TcpConnection新连接创建的时候 TcpConnection=>Channel
                    //因为一个连接对应一个channel，相当于channel握有一个弱智能指针，指向TcpConnection的对象
 ...
 /**
  * 当改变channel所表示fd的events事件后，update负责在poller里面更改fd相应的事件  使用epoll_ctl
  * EventLoop=>ChannelList  Poller
 */
void Channel::update()
{
    //通过channel所属的EventLoop，调用poller的相应方法，注册events事件
